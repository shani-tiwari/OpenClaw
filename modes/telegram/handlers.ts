...
import { clip, commandArgs } from "./text";
import { runAgentTG, runAskTG, runPlanStepsTG } from "./agent-run";
...
export function registerHandler(bot: Telegraf) {
  ...
  bot.command("ask", async (ctx) => {
    ...
    const query = commandArgs(ctx.message.text, "ask");
    if (!query) {
  ...
  bot.command("agent", async (ctx) => {
    ...
    const query = commandArgs(ctx.message.text, "agent");
    if (!query) {
      return ctx.reply("Usage: \n /agent \n <your question>", {
        parse_mode: "Markdown",
      });
  ...
  bot.command("plan", async (ctx) => {
    ...
    const query = commandArgs(ctx.message.text, "plan");
    if (!query) {
      return ctx.reply("Usage: \n /plan \n <your goal.....>", {
        parse_mode: "Markdown",
      });
